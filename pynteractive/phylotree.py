...
class PhyloTree(DataStruct):
	...
	def _refresh(self):
		self._update('setData',str(self.newick))
		for i,j in list(self.features.items()):
			self._update('addPhyloFeat',i,*j)

		for i,j in list(self.tipfeatures.items()):
			for k in j:
		...
		for trackn,data in list(self.tracks.items()):
			for tipname,title in list(data[2].items()):
				self._update('addTreeTrackFeature',trackn,tipname,data[1],title,False)

		for trackn,data in list(self.gradientTracks.items()):
			for tipname,[title,value] in list(data[4].items()):
				self._update('addTreeTrackFeature',trackn,tipname,self.__getHexColor(trackn,value),title,value)

		for trackn,data in list(self.mcolorTracks.items()):
			for tipname,value in list(data[2].items()):
				self._update('addTreeTrackFeature',trackn,tipname,data[1][value],value,value)

		for tipname,color in list(self.cladeMarks.items()):
			self._update("markClade",tipname,color)

		for tipname,color in list(self.cladeColors.items()):
			self._update("setCladeColor",tipname,color)

		self._refreshBars()

	def _refreshBars(self):
		for nbar,(description,color,minval,maxval,bars) in  list(self.trackBars.items()):
			for tipname,value in list(bars.items()):
				rng=maxval-minval
				val=value-minval
				val=rng-val
				val=1-float(val)/rng
	...
	def getTips(self):
		...
		return [i.name for i in list(self.newick.leafs.values())]
...
- color: HTML compatible color '''
	...
	def clearCladeMarks(self):
		...
		for i in list(self.cladeMarks.keys()):
			self.unMarkClade(i)
